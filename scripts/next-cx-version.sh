#!/usr/bin/env bash
# 计算下一个 cx 发布版本号：<opencode版本>-cx.N（US-009 detect 阶段用）。
#
# 规则：
#   - 基线版本取自 brand/BASE_TAG（如 v1.17.15），即当前锁定的 opencode 上游 tag；
#   - 在「已有发布版本」列表里找形如 <base>-cx.<N> 的 tag，取最大 N，下一个为 N+1；
#   - 该基线尚无任何 cx 发布时，从 1 开始（首个发布 v1.17.15-cx.1）。
#
# 已有版本来源（按优先级）：
...
#
# 用法：
#   scripts/next-cx-version.sh                       # 自动读 BASE_TAG + 本地 tag
#   scripts/next-cx-version.sh --base v1.17.15       # 覆盖基线版本
#   EXISTING_TAGS="v1.17.15-cx.1 v1.17.15-cx.2" scripts/next-cx-version.sh --base v1.17.15
#
# 输出（若设置 GITHUB_OUTPUT 则写入其中，同时也打到 stdout 便于本机查看）：
#   base=v1.17.15
#   n=1
#   version=v1.17.15-cx.1
#   tag=v1.17.15-cx.1
#
...
# 已有 cx tag 列表：EXISTING_TAGS 优先，否则本地 git tag。
existing="${EXISTING_TAGS:-}"
if [ -z "$existing" ]; then
  existing="$(git -C "$PROJECT_ROOT" tag -l "${base}-cx.*" 2>/dev/null || true)"
fi

# 从已有 tag 中抽出严格匹配 <base>-cx.<数字> 的 N，取最大值。
max_n=0
if [ -n "$existing" ]; then
  ...
  while IFS= read -r tag; do
    ...
    case "$tag" in
      "${base}-cx."*)
        n="${tag#"${base}-cx."}"
        # n 必须是纯数字，否则跳过（挡掉 v1.17.15-cx.1-rc 之类）。
        case "$n" in
          ''|*[!0-9]*) continue ;;
          *) [ "$n" -gt "$max_n" ] && max_n="$n" ;;
        esac
