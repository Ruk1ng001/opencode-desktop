#!/usr/bin/env bash
# 画布补丁重放脚本：把 brand/canvas-patches/ 下的 .patch 按 NN- 前缀顺序，累积应用到
# infinite-canvas/ submodule（基线应为 brand/CANVAS_BASE_SHA）。是 update-canvas.sh
# 承诺的「apply 脚本」，用于「submodule 锁上游 + 薄叠加层」中画布侧的重放环节。
#
# 与 apply-patches.sh（opencode 侧）逻辑一致，只换目标仓库 / 补丁目录 / 基线文件：
#   SRC_DIR      = infinite-canvas/         （opencode 侧为 opencode/）
#   PATCHES_DIR  = brand/canvas-patches/    （opencode 侧为 brand/patches/）
#   BASE_SHA     = brand/CANVAS_BASE_SHA    （opencode 侧为 brand/BASE_SHA）
#
# 两种模式：
#   scripts/apply-canvas-patches.sh          # 【apply】在 infinite-canvas/ 内真实落地补丁（构建前重放）
#   scripts/apply-canvas-patches.sh --check  # 【check】临时 worktree 里从 BASE_SHA 累积重放做验证，
#                                            #          逐个报告 OK/FAIL，完事删 worktree，绝不碰工作区
#   scripts/apply-canvas-patches.sh -h | --help
#
# 为什么 check 用临时 worktree：补丁可能「按序累积」，单个 `git apply --check` 对干净基线
#   跑会假 FAIL；必须真实按序落地才能验证。worktree 让验证不污染 infinite-canvas/ 当前状态。
#
# 退出码：
#   0  全部补丁应用成功（apply 落地 / check 验证通过）
#   非 0  某补丁应用失败（apply 模式停在首个失败；check 模式跑完全部再汇总失败）
#
# apply / check 的 git apply 语义不同（刻意为之，对齐 opencode 侧）：
#   - apply（构建 / CI 重放）：strict `git apply`，补丁须精确适用，fuzzy 成功=隐患→硬失败；
#   - check（维护者验证）：`git apply --3way`，上下文轻微漂移时三方合并兜底，作诊断信号。
#
# 不变式：不提交、不改 brand/canvas-patches/。apply 模式会弄脏 infinite-canvas/ 工作区
#         （这是重放目的，gitlink 保持指向 BASE_SHA，构建取工作区文件）；check 模式零副作用。
set -euo pipefail
...
# check 模式的临时 worktree 清理钩子。CHECK_WT 可能未定义，用 ${CHECK_WT:-} 兜底避开 set -u。
CHECK_WT=""
cleanup_wt() {
  [ -n "${CHECK_WT:-}" ] || return 0
...
# 按 NN 序列出所有补丁的绝对路径（无补丁则空）。LC_ALL=C 保证稳定字典序（等价 NN 序）。
list_patches() {
...
run_check() {
  ...
  log "check：在临时 worktree 从基线 $base_sha 累积重放（不碰 infinite-canvas/ 工作区）"
  ...
  if [ "$total" -eq 0 ]; then
    warn "brand/canvas-patches/ 下没有 .patch，无补丁可验证。"
    return 0
  fi
  if [ "$fails" -eq 0 ]; then
...
# —— apply 模式：在 infinite-canvas/ 内真实按序落地 ——
run_apply() {
  local base_sha head
  ...
  if [ -n "$base_sha" ] && [ -n "$head" ] && [ "$head" != "$base_sha" ]; then
    warn "infinite-canvas/ HEAD ($head) 与基线 CANVAS_BASE_SHA ($base_sha) 不一致，补丁可能不适用。"
  fi
  ...
  while IFS= read -r p; do
    ...
    if git -C "$SRC_DIR" apply "$p" >/dev/null 2>&1; then
      ...
      die "重放中断于 $name（前序补丁改动已落在 infinite-canvas/ 工作区，可 checkout -f 回退）。"
    fi
  done < <(list_patches)

  [ "$total" -gt 0 ] || { warn "brand/canvas-patches/ 下没有 .patch，无补丁可应用。"; return 0; }
  log "已按序落地 $total 个补丁到 infinite-canvas/（gitlink 仍指向基线，构建取工作区文件）。"
}
