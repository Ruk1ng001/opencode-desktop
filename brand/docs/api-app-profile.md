...
### ✅ 结论（US-008 已确认：`/v1/models` 已按 key 过滤，无需新接口）

经查 new-api 源码（`/srv/new-api/new-api`，与 profile 端点同仓库），`GET /v1/models` **已按 token 的 `model_limits` 过滤**，落到「首选」分支——**App 端直接用 `/v1/models`，不新增 `/api/app/models`**。证据链：

1. **同源鉴权**：`router/relay-router.go` 的 `/v1/models` 路由组挂 `middleware.TokenAuth()`（与 `/v1/*` 同套 token 鉴权），由 `Authorization: Bearer sk-…` 定位 token。
2. **鉴权即注入模型限制**：`TokenAuth()` → `SetupContextForToken()`（`middleware/auth.go`）在 `token.ModelLimitsEnabled` 为真时，向请求上下文写入 `token_model_limit_enabled=true` 与 `token_model_limit=token.GetModelLimitsMap()`（token 的 `model_limits` 白名单）。
3. **handler 按该上下文过滤**：`controller.ListModels()`（`controller/model.go`）读取 `ContextKeyTokenModelLimitEnabled`：
   - 开启限制 → 只把 `token_model_limit` 白名单里的模型放进返回集（`userModelNames`）；
   - 未开启 → 取该 token 绑定分组/渠道（`groups.tokenGroup` / `ownerGroups`）的 `GetGroupEnabledModels` 可用模型集合。
   两种情况都是**按该 key 维度**，不是全站模型表。
4. **返回形态天然兼容**：默认 OpenAI 分支返回 `{"success":true,"object":"list","data":[<OpenAIModels>]}`，`dto.OpenAIModels` 含 `id`/`object`/`created`/`owned_by` 字段，即 `{object:"list", data:[{id,…}]}`——与本契约第 6 节要求的 App 端解析形态一致。

**因此**：`/api/app/models` 备用接口的实现条件（“`/v1/models` 不按 key 区分”）不成立，**不实现**。App 端切换 key 拉可用模型统一调 `GET /v1/models`（带 `Authorization: Bearer sk-…`），消费 `data[].id`。

> 保留下节备用接口契约仅作历史/回退参考；当前不落地。

### 备用接口契约（仅当 `/v1/models` 不按 key 区分时才需实现；US-008 已确认无需实现）
...
- 切换 key 联动模型：调 `GET /v1/models`（带 `Authorization: Bearer sk-…`）拿该 key 可用模型。US-008 已确认 `/v1/models` 按 token `model_limits` 过滤，无需 `/api/app/models`；消费 `data[].id`。
