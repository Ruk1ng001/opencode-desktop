...
- **改** `web/src/router.tsx`（**必改，实读源码修正原「不改 router.tsx」的错误断言**）：`createBrowserRouter([...])` 当前**只传路由数组、未设 basename**，全仓不读 `import.meta.env.BASE_URL`。Vite 的 `base` 仅改产物里 asset/script 的 URL 前缀，**不会**自动设 React Router 的 basename——iframe 以 `oc://renderer/canvas/index.html` 加载时 `pathname=/canvas/index.html`，无 basename 会匹配到 `path:"*"` 的 `NotFound`。修正为 `createBrowserRouter(routes, { basename: import.meta.env.BASE_URL })`，配合构建期 `VITE_BASE=/canvas/`（Vite 注入 `BASE_URL=/canvas/`）。仅一行，但漏了画布 iframe 直接 404。
