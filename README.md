...
| 平台 | 卸载方式 |
|------|----------|
| macOS | 把「应用程序」里的 `Dokng` 拖到废纸篓。清残留配置：删除 `~/Library/Application Support/opencode/` 与 `~/.config/opencode/` |
| Windows | 「设置 → 应用 → 已安装的应用」找到 `Dokng` → 卸载（NSIS 自带卸载器）。清残留配置：删除 `%APPDATA%\opencode\` |
...
发布由 `.github/workflows/release.yml` 完成——单个 workflow 内含五段 job，按触发事件分流：
...
1. **detect**：读 `brand/BASE_TAG` + 已有 GitHub Release，算出下一个版本号
   `<opencode版本>-dokng.N`；登记渠道 Secret 为日志掩码。
...
3. **release**：汇总各平台产物，创建 / 更新对应 tag 的 GitHub Release，合并 `latest*.yml`，
   可选双写到 Cloudflare R2。
...
### 3. 分发托管（Cloudflare R2 + 下载页，可选增强）

默认所有产物只发到 GitHub Release。国内用户直连 GitHub 下载慢，可额外把产物托管到
**Cloudflare R2**（出站流量免费 + 全球 CDN），并用 **Cloudflare Pages** 做一个自动指向
最新版的下载落地页。R2 与 GitHub Release **双写**：R2 作主分发源，GitHub 保留存档。

启用后，客户端自动更新的主源切到 R2（`electron-updater` 的 generic provider 直接读
R2 上的 `latest*.yml`，按 `version` 比对），GitHub 作为回退源仍写进 `app-update.yml`。

**这是渐进增强：不配 R2 Secret 时 CI 的 R2 上传步骤自动跳过，GitHub Release 发布不受影响。**
