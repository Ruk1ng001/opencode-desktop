...
- **账户工作台**：从主界面侧边栏打开「账户」，可查看账户余额、账户 / Token 用量与额度使用率环形图；
  顶部总览将余额和 Token 统计合并展示，避免重复信息。
- **API Keys 管理**：Keys 区域支持独立滚动，可新增、改名、删除和切换激活 Key。新增后立即显示本地结果，
  远端名称、模型和账户数据在后台同步；失败时表单会保留输入并显示原因。
