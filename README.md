...
- **API Keys 管理**：Keys 区域支持独立滚动，可新增、改名、删除和切换激活 Key。未填写本地名称时，
  卡片会使用 `/api/app/profile` 返回的 new-api `token.Name`；每张卡片展示该 Key 的今日 Tokens、今日 / 累计
  费用、可用模型和 Key 额度状态。新增后立即显示本地结果，远端名称、模型和账户数据在后台同步。
